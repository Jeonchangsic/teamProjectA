...
<head>
    ...
    <title>저긴어때</title>
    <link href="<%=request.getContextPath()%>/resources/css/manager_css/managerFestivalMainReg.css" rel="stylesheet"/>
    <!-- 파비콘 -->
	...
	<script>
		function festivalReg(){
			var ftidx = $(".changeColor > input").val();
			var ftCnt = $(".clickEvent > .inputCnt").length;
			if(ftCnt >= 6){
				alert("페스티발 등록은 최대 6개까지만 가능합니다.");
			}else{
				if(ftidx == null){
					alert("등록할 항목을 선택해 주세요.");
				}else{
					$.ajax({
						url:"<%=request.getContextPath()%>/ajax/festivalReg.do",
						type:"get",
						data:"ftidx="+ftidx,
	...
	<!-- 페스티발 삭제 -->
	<script>
		function festivalRegDt(){
			var ftidx = $(".changeColor > input").val();
			if(ftidx == null){
				alert("삭제할 항목을 선택해 주세요.");
			}else{
				$.ajax({
					url:"<%=request.getContextPath()%>/ajax/festivalRegDt.do",
					type:"get",
					data:"ftidx="+ftidx,
					success:function(data){
						location.reload();
	...
	<!-- 페스티발 등록 리스트 미리보기 팝업창 -->
     <script>
     function preview(){
    	var url = "<%=request.getContextPath()%>/manager/festivalListPreview.do";
 		var winWidth = 950;
 	    var winHeight = 270;
		...
		<div class="list_Style">
	    	...
	    	<c:forEach var ="notVO" items="${notVO}">
	    		<div class="select_Div clickEvent">
	    			<input type="hidden" value="${notVO.ftidx}"/>
	    			<p class="clickEvent">[${notVO.ftAddr}]</p> 
	    		${notVO.ftName}
	    		</div>
	   		</c:forEach>
	  	</div>
	   	<div id="btn_div">
	   		<button type="button" onclick="festivalReg()" class="first_btn"><i class="xi-arrow-right"></i></button>
	   		<button type="button" onclick="festivalRegDt()" class="last_btn"><i class="xi-arrow-left"></i></button>
	   	</div>
	   	<div class="list_Style">
	   		<span>등록된 리스트</span>
		    <c:forEach var ="vo" items="${vo}">
	    		<div class="select_Div clickEvent">
	    			<input type="hidden" value="${vo.ftidx}" class="inputCnt"/>
	    			<p class="clickEvent">[${vo.ftAddr}]</p> 
	    		${vo.ftName}
	    		</div>
	    	</c:forEach>
	   	</div>
