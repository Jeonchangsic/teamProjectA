...
<head>
    ...
    <title>저긴어때</title>
    <link href="<%=request.getContextPath()%>/resources/css/manager_css/managerFestivalMf.css" rel="stylesheet"/>
    <!-- 파비콘 -->
	...
	<!-- 위도/경도 추출을 위한 지도 팝업창 -->
     	...
     	$(document).ready(function(){
     		$("select_Style").val("${vo.ftAddr}").prop("selected",true);
     	})
     </script>
</head>
<body>
    <header>
        <h2 class="font_Style">수정하기</h2>
    </header>
    <div id="headLine"></div>
                <!--end header-->
    <form action="festivalMf.do" method="POST">
	    ...
	    <table>
	    	<tr>
	    		...
	    		<td>
	    			<input type="text" name="ftName" value="${vo.ftName}" maxlength="10" placeholder="10자 이내로 작성해주세요." required/>
	    		</td>
	    	</tr>
	    	<tr>
	    		<td class="font_Style font_StyleBold">지역선택</td>
	    		<td>
	    			<select id="select_Style" name="ftAddr">
	    				<option value="${vo.ftAddr}" hidden>${vo.ftAddr}</option>
						<optgroup label="경기도">
	    	...
	    	<tr>
	    		...
	    		<td>
	    			<input type="text" name="ftSubContent" value="${vo.ftSubContent}" maxlength="15" placeholder="15자 이내로 작성해주세요." required/>
	    		</td>
	    	</tr>
	    	<tr>
	    		<td id="middle" class="font_Style font_StyleBold">축제설명</td>
	    		<td id="textarea_Style">
	    			<textarea id="info" name="ftMainContent" cols="30" rows="10" maxlength="300" placeholder="300자 이내로 작성해주세요." required>${vo.ftMainContent}</textarea>
	    		</td>
	    	</tr>
	    	<tr>
	    		<td class="font_Style font_StyleBold">페이지링크</td>
	    		<td>
	    			<input type="text" name="ftLink" value="${vo.ftLink}" maxlength="400" placeholder="예: http://www.chunhyang.org" required/>
	    		</td>
	    	</tr>
	    	<tr>
	    		<td class="font_Style font_StyleBold">지도좌표</td>
	    		<td>
	    			<input id="map_Input" name="ftLatitude" type="text" value="${vo.ftLatitude}" placeholder="위도를 입력하세요." maxlength="480" required/>
	    			<input id="map_Input2" name="ftLongitude" type="text" value="${vo.ftLongitude}" placeholder="경도를 입력하세요." maxlength="480" required/>
	    	...
	    	<tr>
	    		...
	    		<td>
	    			<input id="file_Input" type="file" name="ftImgName" required/>
	    		</td>
	    	</tr>
	    </table>
	    <div id="btn_Area">
			<button id="btn_Style">완료</button>
	    </div>
    </form>
</body>
</html>