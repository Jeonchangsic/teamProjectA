...
<head>
    ...
    <title>저긴어때</title>
    <link href="<%=request.getContextPath()%>/resources/css/manager_css/managerTripMf.css" rel="stylesheet"/>
    <!-- 파비콘 -->
    ...
    <!---- jQuery ---->
     ...
     <script>
     	function file_Change(){
     		$("#readFile_Input").css("display","none");
     		$("#readFile_Btn").css("display","none");
     		$("#file_Input").prop("type", "file");
     	}
     </script>
     <script>
     	$(document).ready(function(){
     		$("select_Style").val("${vo.tAddr}").prop("selected",true);
     	})
     </script>
</head>
<body>
    ...
    <input type="hidden" name="tidx" value="${vo.tidx}"/>
	    <table>
	    	<tr>
	    		...
	    		<td>
	    			<input type="text" name="tName" value="${vo.tName}" maxlength="10" placeholder="10자 이내로 작성해주세요." required/>
	    		</td>
	    	</tr>
	    	<tr>
	    		...
	    		<td>
	    			<select id="select_Style" name="tAddr">
	    				<option value="${vo.tAddr}" hidden>${vo.tAddr}</option>
						<optgroup label="경기도">
	    	...
	    	<tr>
	    		...
	    		<td>
	    			<input type="text" name="tSubContent" value="${vo.tSubContent}" maxlength="15" placeholder="15자 이내로 작성해주세요." required/>
	    		</td>
	    	</tr>
	    	<tr>
	    		<td id="middle" class="font_Style font_StyleBold">여행지설명</td>
	    		<td id="textarea_Style">
	    			<textarea id="info" name="tMainContent" cols="30" rows="10" maxlength="300" placeholder="300자 이내로 작성해주세요." required>${vo.tMainContent}</textarea>
	    		</td>
	    	</tr>
	    	<tr>
	    		<td class="font_Style font_StyleBold">페이지링크</td>
	    		<td>
	    			<input type="text" name="tLink" value="${vo.tLink}" maxlength="400" placeholder="예: http://www.chunhyang.org" required/>
	    		</td>
	    	</tr>
	    	<tr>
	    		<td class="font_Style font_StyleBold" >지도좌표</td>
	    		<td>
	    			<input id="map_Input" name="tLatitude" type="text" value="${vo.tLatitude}" placeholder="위도를 입력하세요." maxlength="480" required/>
	    			<input id="map_Input2" name="tLongitude" type="text" value="${vo.tLongitude}" placeholder="경도를 입력하세요." maxlength="480" required/>
	    	...
	    	<tr>
	    		...
	    		<td>
	    			<input id="file_Input" type="file" name="tImgName" required/>
	    		</td>
	    	</tr>
	    </table>
	    <div id="btn_Area">
