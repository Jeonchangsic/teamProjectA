...
<script>
	...
	function change_btn2(event,type) {
		...
		location.href="<%=request.getContextPath()%>/lodging/lodgingList_gh.do?type="+type;
	}
				...
				<%--<c:forEach var="vo1" items="${list1}" varStatus="status">
						...
						<c:forEach var="vo2" items="${list2}" varStatus="status">
							...
							<c:if test="${vo1.lidx == vo2.lidx && vo2.rnum > 0}">
								<a
									href="<%=request.getContextPath() %>/lodging/lodgingView.do?lidx=${vo2.lidx}">
									<div class="imgbor">
										<div class="img_left">
											<ul>
												<li class="up_img">
										...
										<img
											src="<%=request.getContextPath()%>/resources/images/lodging_images/${vo1.limagename}">
									</div>
								</a>
								<c:set var="doneLoop" value="true" />
								<!-- doneLoop 논리값 반대 설정 = break -->
